      ...
      <AUtableBody>
        {data.map((row, rowIndex) => (
          <AUtableRow
            key={rowIndex}
            className={`${
              (1 + rowIndex) % rowSpanInterval === 0
                ? "au-table__row--border-bottom"
                : ""
            }`}
          >
...
export const AUtableCell = ({
  ...
  return (
    <td
      className={`au-table__cell ${className} ${
...
export const AUtableRow = ({ children, className, ...attributeOptions }) => {
  return (
    <tr className={`au-table__row ${className}`} {...attributeOptions}>
      {children}
    </tr>
  );
};
...
export const AUtableResponsiveWrapper = ({ children }) => {
  return <div className="au-table__wrapper">{children}</div>;
};
